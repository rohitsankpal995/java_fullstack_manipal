...
import com.rohit.jpa.domain.Mobile;
...
public interface SimpleJpaRepo extends JpaRepository<Mobile, Long> {
    ...
    @Query(value = "select * from mobile", nativeQuery = true)
        // sql
    List<Mobile> findAllSQL();

    @Query(value = "select * from mobile where mob_num = :mobile", nativeQuery = true)
        // sql
    Optional<Mobile> findOneWithMobile1(@Param("mobile") String mobile);

    @Query(value = "select * from mobile where mob_num = ?1", nativeQuery = true)
        // sql
    Optional<Mobile> findOneWithMobile2(String mobile);

    @Query(value = "select m from Mobile m where m.number = ?1")
        // sql
